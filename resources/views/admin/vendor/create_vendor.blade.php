...
<section class="section">
    ...
    <div class="section-body">
        ...
        <form class="container-fuild" action="{{ url('admin/vendor') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="email" name="email_id" value="{{ old('email_id') }}" class="form-control @error('email_id') is_invalide @enderror" placeholder="{{__('Email Address')}}">
    ...
    <div class="card mt-2">
        <div class="card-header">
            <h6 class="c-grey-900">{{__('Select Tags (For Restaurants it will be cuisines)')}}<span class="text-danger">&nbsp;*</span></h6>
        </div>
        <div class="card-body">
            <div class="mT-30">
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <select class="select2 form-control" name="cuisine_id[]" multiple="true">
                            @foreach ($cuisines as $cuisine)
                                <option value="{{ $cuisine->id }}" {{ (collect(old('cuisine_id'))->contains($cuisine->id)) ? 'selected':'' }}>{{ $cuisine->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="row">
                            <div class="pac-card col-md-12 mb-3" id="pac-card">
                                ...
                                <div id="pac-container">
                                    <input id="pac-input" type="text" name="map_address" value="{{ old('map_address') }}" class="form-control" placeholder="Enter A Location" />
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <input type="text" class="form-control" name="address" value="{{ old('address') }}" placeholder="{{__('Vendor Full Address')}}" id="location">
                            </div>
                        </div>
                    </div>
                    <div class="col-md-6">
            ...
            <div class="mT-30">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" min=1 name="min_order_amount" placeholder="{{__('Minimum Order Amount')}}" value="{{ old('min_order_amount') }}" required class="form-control @error('min_order_amount') invalid @enderror">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" min=1 name="for_two_person" placeholder="{{__('Cost Of Two Person')}}" value="{{ old('for_two_person') }}" required class="form-control  @error('for_two_person') invalid @enderror">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" min=1 name="avg_delivery_time" value="{{ old('avg_delivery_time') }}" placeholder="{{__('Avg Delivery Time(in min)')}}" required class="form-control @error('avg_delivery_time') invalid @enderror">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="license_number" value="{{ old('license_number') }}" required placeholder="{{__('Business License Number')}}" class="form-control @error('license_number') is-invalid @enderror">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="admin_comission_type" class="form-control">
                            <option value="amount" {{ (collect(old('admin_comission_type'))->contains('amount')) ? 'selected':'' }}>{{__('Amount')}}</option>
                            <option value="percentage" {{ (collect(old('admin_comission_type'))->contains('percentage')) ? 'selected':'' }}>{{__('Percenatge')}}</option>
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="admin_comission_value" value="{{ old('admin_comission_value') }}" placeholder="{{__('Admin Comission Value')}}" required class="form-control">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="vendor_type" class="form-control">
                            <option value="all" {{ (collect(old('vendor_type'))->contains('all')) ? 'selected':'' }}>
                                {{__('All')}}</option>
                            <option value="veg" {{ (collect(old('vendor_type'))->contains('veg')) ? 'selected':'' }}>
                                {{__('pure veg')}}</option>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="time_slot" class="form-control">
                            <option value="15" {{ (collect(old('time_slot'))->contains('15')) ? 'selected':'' }}>
                                {{__('15 mins')}}</option>
                            <option value="30" {{ (collect(old('time_slot'))->contains('30')) ? 'selected':'' }}>
                                {{__('30 mins')}}</option>
                            <option value="45" {{ (collect(old('time_slot'))->contains('45')) ? 'selected':'' }}>
                                {{__('45 mins')}}</option>
                            <option value="60" {{ (collect(old('time_slot'))->contains('60')) ? 'selected':'' }}>
                                {{__('1 hour')}}</option>
                            <option value="120" {{ (collect(old('time_slot'))->contains('120')) ? 'selected':'' }}>
                                {{__('2 hour')}}</option>
                            <option value="180" {{ (collect(old('time_slot'))->contains('180')) ? 'selected':'' }}>
                                {{__('3 hour')}}</option>
                            <option value="240" {{ (collect(old('time_slot'))->contains('240')) ? 'selected':'' }}>
                                {{__('4 hour')}}</option>
                            <option value="300" {{ (collect(old('time_slot'))->contains('300')) ? 'selected':'' }}>
                                {{__('5 hour')}}</option>
                        </select>
                    </div>
                </div>
