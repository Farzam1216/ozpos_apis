...
<section class="section">
    <div class="section-header">
        <h1>{{ $submenu->name }}&nbsp;{{__('Customise')}}</h1>
        <div class="section-header-breadcrumb">
            <div class="breadcrumb-item active"><a href="{{ url('admin/home') }}">{{__('Dashboard')}}</a></div>
            <div class="breadcrumb-item active"><a href="{{ url('admin/vendor/'.$submenu->vendor_id)}}">{{App\Models\Vendor::find($submenu->vendor_id)->name}}</a></div>
            <div class="breadcrumb-item active"><a href="{{ url('admin/menu/'.$submenu->menu_id)}}">{{App\Models\Menu::find($submenu->menu_id)->name}}</a></div>
    ...
    <div class="section-body">
        ...
        <div class="card">
            <div class="card-header">
                <h4>{{__('Customise')}}</h4>
                <div class="w-100">
                    @can('admin_custimization_type_add')
                    <button type="button" class="btn btn-primary float-right" data-toggle="modal"
                        data-target="#staticBackdrop">{{__('Add customization type')}}
                    </button>
                    @endcan
                </div>
            </div>

            @can('admin_custimization_type_access')
                <div class="card-body">
                    <div id="accordion">
                        @foreach ($custimization_types as $custimization_type)
                            <div class="accordion">
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        @can('admin_custimization_type_edit')
                                        <span>
                                        ...
                                        @endcan
                                        @can('admin_custimization_type_delete')
                                        <span>
                                            ...
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <td>{{__('Default')}}<span class="text-danger">&nbsp;*</span></td>
                                                        <td>{{__('Active')}}<span class="text-danger">&nbsp;*</span></td>
                                                        <td></td>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                        ...
                                                        <tbody id="{{'custom'.$custimization_type->id}}">
                                                            ...
                                                            @if ($items)
                                                                @foreach ($items as $item)
                                                                    <tr>
                                                                        <td>
                                                                            <input type="text" name="{{str_replace(' ', '_', strtolower('name'.$custimization_type->name.'[]'))}}" placeholder="{{__('Item name')}}" value="{{ $item->name }}" class="form-control">
                                                                        </td>
                                                                        <td>
                                                                            <input type="number" step="0.01" min="0.01" name="price[]" value="{{ $item->price }}" placeholder="{{__('Price')}}" class="form-control">
                                                                        </td>
                                                                        <td>
                                                            ...
                                                            @else
                                                                <tr id="{{'custom'.$custimization_type->id}}">
                                                                    <td>
                                                                        <input type="text" required name="{{str_replace(' ', '_', strtolower('name'.$custimization_type->name.'[]'))}}" placeholder="{{__('Item name')}}" class="form-control">
                                                                    </td>
                                                                    <td>
                                                                        <input type="number" step="0.01" min="0.01" required name="price[]" placeholder="{{__('Price')}}" class="form-control">
                                                                    </td>
                                                                    <td>
        ...
        <div class="modal-content">
            <form action="{{ url('admin/customization_type') }}" method="post">
                ...
                <div class="modal-body">
                    <div class="row mt-5">
                        <div class="col-md-12">
                            <label for="">{{__('Name of Custimization type')}}</label>
                    ...
                    <div class="row mt-5">
                        <div class="col-md-12">
                            <label for="">{{__('Minimum Item selection')}}</label>
                    ...
                    <div class="row mt-5">
                        <div class="col-md-12">
                            <label for="">{{__('Maximum Item selection')}}</label>
