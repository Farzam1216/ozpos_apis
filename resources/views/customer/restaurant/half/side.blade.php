<div id="HalfMenu-{{ $prefix }}-{{ $HalfNHalfMenu->id }}-{{ $ItemSize->id }}" class="tab-pane fade @if($prefix=='First') show in active @endif " style="background-color: unset;">
   ...
   @foreach($HalfNHalfMenu->ItemCategory()->get()->first()->SingleMenuItemCategory()->get() as $SingleMenuItemCategoryIDX=>$SingleMenuItemCategory)
      ...
      <script type="text/javascript">
         $(document).on('click', "#HalfMenu-{{ $prefix }}-{{ $HalfNHalfMenu->id }}-{{ $ItemSize->id }}-{{ $Menu->id }}", function () {
            console.log('clicked');
         ...
         $(document).on('change', ".HalfMenuCheckbox-{{ $prefix }}-{{ $HalfNHalfMenu->id }}-{{ $ItemSize->id }}-{{ $Menu->id }}", function () {
         ...
         <div class="p-3 border-bottom menu-list">
            ...
            @if($MenuSize !== NULL && $MenuSize->MenuAddon()->get()->count() !== 0)
               <span class="float-right">
                  <button class="btn btn-outline-secondary btn-sm " onclick="HalfMenuAddon('{{ $prefix }}','{{ $HalfNHalfMenu->id }}','{{ $ItemSize->id }}','{{ $Menu->id }}')">
                     Edit
                  </button>
               </span>
