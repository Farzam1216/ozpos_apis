...
@section('content')
  ...
  <section class="section">
    ...
    <div class="card">
      ...
      @endif
      <div class="card-body">
        <form class="container-fuild" action="{{ url('vendor/promo_code/'.$promoCode->id) }}" method="post"
          ...
          <div class="row">
            <div class="col-md-6 mb-5">
              ...
              <input type="text" name="name" class="form-control @error('name') is_invalide @enderror"
                     placeholder="{{__('Promo Code Name')}}" value="{{ $promoCode->name }}" required="">
              @error('name')
          ...
          <div class="row">
            <div class="col-md-6 mb-5" style="display: none;">
              <label for="vendor">{{__('Promo code valid for this Vendor')}}<span
                  class="text-danger">&nbsp;*</span></label>
              <select name="vendor_id[]" class="select2 form-control" multiple>
                @foreach ($vendors as $vendor)
                  <option
                    value="{{$vendor->id}}" {{in_array($vendor->id,explode(',',$promoCode->vendor_id)) ? 'selected' : ''}}>{{ $vendor->name }}</option>
                @endforeach
              </select>
            </div>

            <div class="col-md-12 mb-5">
              <label for="vendor">{{__('Promo code valid for this Customer')}}<span
                  class="text-danger">&nbsp;*</span></label>
              <select name="customer_id[]" class="select2 form-control" multiple>
                @foreach ($users as $user)
                  <option
                    value="{{$user->id}}" {{in_array($user->id,explode(',',$promoCode->customer_id)) ? 'selected' : ''}}>{{ $user->name }}</option>
                @endforeach
              </select>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-3 mb-5 mt-3">
              <input type="checkbox" id="customer_app"
                     {{($promoCode->display_customer_app == 1) ? 'checked' : ''}} name="display_customer_app">
              <label for="customer_app">{{__('Show in customer App')}}</label>
            </div>

            <div class="col-md-3 mb-5 mt-3" style="display: none;">
            ...
            <div class="col-md-3 mb-5 {{($promoCode->isFlat == 1) ? 'hide' : ''}} discountType">
              ...
              <select name="discountType" id="Discount type" class="form-control">
                <option
                  value="percentage" {{$promoCode->discountType == 'percentage' ? 'selected' : ''}}>{{__('percentage')}}</option>
                <option
                  value="amount" {{$promoCode->discountType == 'amount' ? 'selected' : ''}}>{{__('amount')}}</option>
              </select>
            </div>
            ...
            <div class="col-md-3 mb-5 {{($promoCode->isFlat == 1) ? 'hide' : ''}} max_disc_amount">
              ...
              <input type="number" min=0 name="discount" class="form-control @error('discount') is_invalide @enderror"
                     id="Maximum Discount amount" placeholder="{{__('Enter Discount')}}"
            ...
            <div class="col-md-6 mb-5 {{($promoCode->isFlat == 0) ? 'hide' : ''}} flatDiscount" style="display: none;">
              <label for="{{__('Flat Discount')}}">{{__('Flat Discount')}}</label>
              <input type="number" min=0 name="flatDiscount"
                     class="form-control @error('flatDiscount') is_invalide @enderror" id="flatDiscount"
          ...
          <div class="row">
            <div class="col-md-12 mb-5">
              ...
              <input type="number" min=1 name="max_disc_amount" value="{{ $promoCode->max_disc_amount }}"
                     class="form-control" required placeholder="{{__('Enter Maximum Discount')}}"
                     value="{{ old('discount') }}">
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6 mb-5">
              <label
                for="{{__('Promo Code redeem start/end Period')}}">{{__('Promo Code redeem start/end Period')}}</label><br>
              <input type="text" value="{{ $promoCode->start_end_date }}" id="update_start_end_date"
                     name="update_start_end_date" class="form-control"/>
            </div>
            <div class="col-md-6 mb-5">
          ...
          <div class="row">
            <div class="col-md-6 mb-5">
              <label for="{{__('min_order_amount')}}">{{__('Minimum order amount')}}<span
                  class="text-danger">&nbsp;*</span></label>
            ...
            <div class="col-md-6 mb-5">
              <label for="{{__('max_user')}}">{{__('Coupon Valid For First X User')}}<span
                  class="text-danger">&nbsp;*</span></label>
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-5">
              ...
              <select class="form-control" name="coupen_type">
                ...
                <option
                  value="delivery" {{$promoCode->coupen_type == 'delivery' ? 'selected' : ''}}>{{__('delivery')}}</option>
                <option
                  value="pickup" {{$promoCode->coupen_type == 'pickup' ? 'selected' : ''}}>{{__('pickup')}}</option>
              </select>
              @error('max_order')
              <span class="custom_error" role="alert">
                                <strong>{{ $message }}</strong>
          ...
          <div class="row">
            <div class="col-md-6 mb-5">
              <label for="{{__('max_user')}}">{{__('Promo code description')}}<span
                  class="text-danger">&nbsp;*</span></label>
              <textarea name="description" class="form-control"
                        placeholder="{{__('Promo Code Description')}}">{{ $promoCode->description }}</textarea>
            </div>
            <div class="col-md-6 mb-5">
              <label for="{{__('max_user')}}">{{__('Promo code display text')}}<span class="text-danger">&nbsp;*</span></label>
              <textarea name="display_text" class="form-control"
                        placeholder="{{__('Promo Code Display Text')}}">{{ $promoCode->display_text }}</textarea>
            </div>
          </div>
