    ...
    <div class="section-header">
      ...
      <div class="section-header-breadcrumb">
        <div class="section-header-breadcrumb">
          ...
          <div class="breadcrumb-item active"><a href="{{ url('vendor/addon_category') }}">Addon Category</a></div>
    ...
    <div class="section-body">
      ...
      <div class="card">
          ...
          <table id="datatable" class="table table-striped table-bordered text-center" cellspacing="0" width="100%">
            ...
            @foreach ($Addon as $idx=>$item)
              <tr>
                ...
                <td>{{$item->name}}</td>
                <td>
                  <button type="button" onclick="updateData('vendor/addon',{{$item->id}})" class="btn btn-primary" data-toggle="modal" data-target="#edit_modal">
                    <i class="fas fa-pencil-alt"></i>
                  </button>
                  <a href="javascript:void(0);" class="table-action btn btn-danger btn-action" onclick="deleteData('vendor/addon',{{ $item->id }},'{{ $title }}')">
        ...
        <div class="card-footer">
          <input type="button" value="Delete selected" onclick="deleteSelectionData('vendor/addon/selection_destroy','{{ $title }}')" class="btn btn-primary">
        </div>
      </div>
    </div>
  ...
  <div class="modal right fade" id="insert_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <form class="container-fuild" action="{{ url('vendor/addon') }}" method="post" enctype="multipart/form-data">
          @csrf
          <input type="hidden" name="old_value" value="add">
          <input type="hidden" name="vendor_id" value="{{ $Vendor->id }}">
          <input type="hidden" name="addon_category_id" value="{{ $addon_category_id }}">
