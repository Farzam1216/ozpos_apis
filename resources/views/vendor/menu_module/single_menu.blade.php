         ...
         <div class="card">
            ...
            <div class="card-body">
               <table id="datatable" class="table table-striped table-bordered text-center" cellspacing="0" width="100%">
                  ...
                  <tbody>
                  @foreach ($SingleMenu as $idx=>$item)
                     <tr>
                        <td>
                        ...
                        <td>
                           <button type="button" onclick="updateData('vendor/single_menu',{{$item->id}})" class="btn btn-primary" data-toggle="modal" data-target="#edit_modal">
                              <i class="fas fa-pencil-alt"></i>
                           </button>
                           <a href="javascript:void(0);" class="table-action btn btn-danger btn-action" onclick="deleteData('vendor/single_menu',{{ $item->id }},'{{ $title }}')">
            ...
            <div class="card-footer">
               <input type="button" value="Delete selected" onclick="deleteSelectionData('vendor/single_menu/selection_destroy','{{ $title }}')" class="btn btn-primary">
            </div>
         </div>
      </div>
   ...
   <div class="modal right fade" id="insert_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2">
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            <form class="container-fuild" action="{{ url('vendor/single_menu') }}" method="post" enctype="multipart/form-data">
               @csrf
               ...
               <div class="modal-body">
                  <div class="form-group">
                     <label class="form-control-label">Menu<span class="text-danger">&nbsp;*</span></label>
                     <select class="form-control @error('menu_id') is-invalid @enderror" name="menu_id">
                        @foreach(App\Models\Menu::where('vendor_id', $Vendor->id)->get() as $Menu)
                           <option value="{{ $Menu->id }}" @if(old('menu_id') === $Menu->id) selected @endif >{{ $Menu->name }}</option>
                        @endforeach
                     </select>
                     @error('menu_id')
                  ...
                  <div class="form-group">
                     <label class="form-control-label">Item Categories<span class="text-danger">&nbsp;*</span></label>
                     <select class="form-control @error('item_categories') is-invalid @enderror" name="item_categories[]" multiple style="height: 70px;">
         ...
         <div class="modal-content">
            <form class="container-fuild" id="update_form" method="post" enctype="multipart/form-data">
               ...
               <div class="modal-body">
                  <div class="form-group">
                     <label class="form-control-label">Menu<span class="text-danger">&nbsp;*</span></label>
                     <select class="populate form-control @error('menu_id') is-invalid @enderror" name="menu_id">
                        @foreach(App\Models\Menu::where('vendor_id', $Vendor->id)->get() as $Menu)
                           <option value="{{ $Menu->id }}" @if(old('menu_id') === $Menu->id) selected @endif >{{ $Menu->name }}</option>
                        @endforeach
                     </select>
                  ...
                  <div class="form-group">
                     ...
                     <label for="update_status" class="form-control-label">Status<span class="text-danger">&nbsp;*</span></label>
                     @error('status')
                     <span class="invalid-feedback" role="alert"> <strong>{{ $message }}</strong> </span>
                     @enderror
                  </div>
                  <div class="form-group">
                     <label class="form-control-label">Item Categories<span class="text-danger">&nbsp;*</span></label>
                     <select class="populate form-control @error('item_categories') is-invalid @enderror" name="item_categories[]" multiple style="height: 70px;">
                        @foreach(App\Models\ItemCategory::where('vendor_id', $Vendor->id)->get() as $ItemCategory)
                           <option value="{{ $ItemCategory->id }}" @if(old('item_categories') === $ItemCategory->id) selected @endif >{{ $ItemCategory->name }}</option>
                        @endforeach
                     </select>
