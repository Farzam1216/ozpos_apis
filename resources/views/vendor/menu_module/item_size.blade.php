@php
   $title = 'Item Size';
@endphp@extends('layouts.app',['activePage' => 'vendor_item_size'])
      ...
      <div class="section-body">
         ...
         <div class="card">
               ...
               <table id="datatable" class="table table-striped table-bordered text-center" cellspacing="0" width="100%">
                  ...
                  @foreach ($ItemSize as $idx=>$item)
                     <tr>
                        ...
                        <td>{{$item->name}}</td>
                        <td>
                           <button type="button" onclick="updateData('vendor/item_size',{{$item->id}})" class="btn btn-primary" data-toggle="modal" data-target="#edit_modal">
                              <i class="fas fa-pencil-alt"></i>
                           </button>
                           <a href="javascript:void(0);" class="table-action btn btn-danger btn-action" onclick="deleteData('vendor/item_size',{{ $item->id }},'{{ $title }}')">
            ...
            <div class="card-footer">
               <input type="button" value="Delete selected" onclick="deleteSelectionData('vendor/item_size/selection_destroy','{{ $title }}')" class="btn btn-primary">
            </div>
         </div>
      </div>
   ...
   <div class="modal right fade" id="insert_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel2">
      <div class="modal-dialog" role="document">
         <div class="modal-content">
            <form class="container-fuild" action="{{ url('vendor/item_size') }}" method="post" enctype="multipart/form-data">
               @csrf
